...
NeuralNetwork::NeuralNetwork(const vector<uint> &topology, const RowVector &inputScaling, Scalar learningRate)
{
	...
	for (uint i = 0; i < topology.size(); ++i)
	{
		...
		if (i == topology.size() - 1)
			neuronLayers.push_back(new RowVector(
			        topology[i])); // we are using vectors here as layors and not 2d matrices, we are using stohastic gradient descent
		else
		...
		if (i > 0)
		{
			if (i != topology.size() - 1)
			{
				...
				weights.back()
				        ->col(topology[i])
				        .setZero(); // set all elements to zero in the last column of the weights matrix
				weights.back()->coeffRef(topology[i - 1], topology[i]) =
				        1.0; // except for the last element -> this is due to the bias neuron
			}
			else
			{
...
RowVector NeuralNetwork::propagateForward(const RowVector &input)
{
	...
	scaled_input = input.array() *
	               inputScaling.array(); // Eigen allows assigning array expressions to matrix / vector variables
...
int NeuralNetwork::loadWeights(const string &filename)
{
	...
	for (Matrix *p : weights)
	{ // a check is missing here to check if the structure of the neural network is matching or not
		getline(file, line, '\n');
